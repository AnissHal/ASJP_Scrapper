...
def test_performance():
	...
	requests.get('http://localhost:5000/revue/455')
	benchmark['Revue fetch'] = int((time.perf_counter() - start_time) )

	start_time = time.perf_counter()
	requests.get('http://localhost:5000/455')
	benchmark['Article fetch'] = int((time.perf_counter() - start_time) )

	start_time = time.perf_counter()
	requests.get('http://localhost:5000/search_revue/1?query=revue')
	benchmark['Search Revue fetch'] = int((time.perf_counter() - start_time) )

	start_time = time.perf_counter()
	requests.get('http://localhost:5000/search/technologie/1')
	benchmark['Search Article fetch'] = int((time.perf_counter() - start_time) )

	start_time = time.perf_counter()
	requests.get('http://localhost:5000/revue_article/455')
